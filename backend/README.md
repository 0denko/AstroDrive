...
The web UI sends commands to FastAPI. FastAPI forwards them over USB serial using `enable`, `disable`, `stop`, and `move <ra|dec> <forward|backward> <steps>`. MQTT is an additional command publication path for other services. The serial port can be changed from the web UI; it is stored in `/var/lib/astrodrive/serial-config.json` and takes effect immediately.

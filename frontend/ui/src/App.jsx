...
export default function App() {
  ...
  return <main>
    ...
    {view === 'control' && <section className="grid">
      <div className="panel controls"><div className="panel-heading"><span>Manual control</span><span>{status?.serial_port ?? 'auto'}</span></div><div className="dpad"><button onClick={() => sendCommand({ command: 'move', axis: 'dec', direction: 'forward', steps: 10 })}>{axisLabel.dec} +</button><button onClick={() => sendCommand({ command: 'move', axis: 'ra', direction: 'backward', steps: 10 })}>{axisLabel.ra} −</button><button onClick={() => sendCommand({ command: 'stop' })}>STOP</button><button onClick={() => sendCommand({ command: 'move', axis: 'ra', direction: 'forward', steps: 10 })}>{axisLabel.ra} +</button><button onClick={() => sendCommand({ command: 'move', axis: 'dec', direction: 'backward', steps: 10 })}>{axisLabel.dec} −</button></div><div className="actions"><button onClick={() => sendCommand({ command: 'enable' })}>Enable motors</button><button onClick={() => sendCommand({ command: 'disable' })}>Disable</button></div><form onSubmit={saveSerialPort}><label>ESP32 serial port<input value={serialPort} placeholder="auto or /dev/ttyACM0" onChange={(event) => setSerialPort(event.target.value)} /></label><span className="hint">{status?.esp32_connected ? `Open on ${status.serial_device || 'unknown device'}` : 'No serial device found'}</span><button className="primary" type="submit">Save and reconnect</button></form></div>

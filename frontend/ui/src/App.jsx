...
export default function App() {
  ...
  return <main>
    <header><div><span className="eyebrow">MOUNT CONTROL / LIVE</span><h1>AstroDrive</h1></div><div className="status-stack"><span className={`connection ${status?.esp32_connected ? 'online' : ''}`}>{status?.esp32_connected ? 'ESP32 ONLINE' : 'ESP32 OFFLINE'}</span><span className="mount-state">MOUNT {status?.mount?.toUpperCase() ?? 'CHECKING'}</span></div></header>

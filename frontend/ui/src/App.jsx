...
export default function App() {
  ...
  const [stack, setStack] = useState({ frames: 8, interval_ms: 250, stretch: 2 });
  ...
  async function captureStack(event) {
    ...
    try {
      await request('/api/camera/stack', { method: 'POST', body: JSON.stringify({ ...stack, frames: Number(stack.frames), interval_ms: Number(stack.interval_ms), stretch: Number(stack.stretch) }) });
      for (;;) {
        ...
        setMessage(result.state === 'failed' ? `Stack failed: ${result.detail}` : 'Stacked frame ready');
        break;
      }
